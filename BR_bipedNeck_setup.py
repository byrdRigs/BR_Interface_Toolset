...
Title
Description
	This file contains: Biped Neck Setup
	
How to run:
	import BR_Interface_Toolset.BR_bipedNeck_setup as biNeck
	reload (biNeck)
	biNeck.gui()
'''
...
def gui():
	if pm.window('BR_bipedNeck_setup', q=1, exists=1):
		pm.deleteUI('BR_bipedNeck_setup')
		#BR_bipedNeck_setup

	global system_name

	win_width = 280
	window_object = pm.window('BR_bipedNeck_setup', title="BR_bipedNeck_setup", w=win_width, bgc=window_bgc)
	main_layout = pm.columnLayout()

	system_name = pm.textFieldGrp( l='System Name', text='neck', w=win_width )
	pm.button( l='Go', w=win_width, c=neckSetup )

	pm.window('BR_bipedNeck_setup', e=1, wh=(280, 80), rtf=1)
...
def neckSetup(*args):
	...
	# Check to see if there are 4 or more/less joints in the neck
	if limbJoints == 4:
		pass

	if limbJoints < 4 :
		pm.error( 'Please make 4 neck joints' )

	if limbJoints >= 5:
		pm.error( 'Please make 4 nack joints' )
	...
	pos = pm.xform( jointHierarchy[3], q=1, t=1, ws=1 )
	pm.joint( p=pos, n=jointHierarchy[1] + '_fk' )

	pm.joint(jointHierarchy[0] + '_fk', zso=1, ch=1, e=1, oj='xyz', secondaryAxisOrient='zup')
	pm.joint(jointHierarchy[1] + '_fk', zso=1, ch=1, e=1, oj='none')

	pm.circle( nr=(1, 0, 0), n=limbName + '_fk_icon', ch=0 )
	fk_icon = pm.ls(sl=1)[0]
	temp_constraint = pm.parentConstraint( jointHierarchy[1] + '_ik', fk_icon, mo=0 )
	pm.delete( temp_constraint )
	freezeTransform()

	pos = jointRoot.getTranslation( ws=1 )
	fk_icon.setPivots( pos )

	fk_local = pm.group( em=1, n=jointHierarchy[0] + '_fk_local')
	temp_constraint = pm.parentConstraint( fk_icon, fk_local, mo=0 )
	pm.delete( temp_constraint )

	pm.parent( fk_icon, fk_local )
	freezeTransform()

	pm.orientConstraint( fk_icon, jointHierarchy[0] + '_fk', mo=1 )
 
	fk_icon.overrideEnabled.set( 1 )
	fk_icon.overrideColor.set( 18 )
	...
	# Create spline ik 
	ikh = pm.ikHandle( sol='ikSplineSolver', ee=jointHierarchy[3] + '_ik', sj=jointHierarchy[0] + '_ik', c=ik_crv, ccv=0, n=limbName + '_ikh' )[0]

	# Create the crv bind joints 
	pm.select( cl=1 )
	pos = pm.xform( jointRoot, q=1, t=1, ws=1 )
	pm.joint( p=pos, n=jointHierarchy[0] + '_crvBind' )

	pos = pm.xform( jointHierarchy[3], q=1, t=1, ws=1 )
	pm.joint( p=pos, n=jointHierarchy[1] + '_crvBind' )

	pm.joint(jointHierarchy[0] + '_crvBind', zso=1, ch=1, e=1, oj='xyz', secondaryAxisOrient='zup')
	pm.joint(jointHierarchy[1] + '_crvBind', zso=1, ch=1, e=1, oj='none')

	pm.parent( jointHierarchy[1] + '_crvBind', w=1 )

	pm.select( jointHierarchy[0] + '_crvBind', jointHierarchy[1] + '_crvBind', ik_crv )
	pm.mel.SmoothBindSkin()


	pm.circle( nr=(1, 0, 0), n=limbName + '_icon', ch=0 )
	ik_icon = pm.ls( sl=1 )[0]

	temp_constraint = pm.parentConstraint( jointHierarchy[2] , ik_icon, mo=0 )
	pm.delete( temp_constraint )
	freezeTransform()

	selection = pm.ls( jointHierarchy[3] + '_ik' )[0]

	pos = selection.getTranslation( ws=1 )
	ik_icon.setPivots( pos )

	ik_local = pm.group( em=1, n=limbName + '_ik_local' )
	temp_constraint = pm.parentConstraint( jointHierarchy[2], ik_local, mo=0 )
	pm.delete( temp_constraint )

	pm.parent( ik_icon, ik_local )
	freezeTransform()

	ik_icon.overrideEnabled.set( 1 )
	ik_icon.overrideColor.set( 17 )

	pm.parentConstraint( ik_icon, jointHierarchy[1] + '_crvBind', mo=1 )
	...
	pm.connectAttr( jointHierarchy[0] + '_crvBind.xformMatrix', ikh + '.dWorldUpMatrix', f=1 )
	pm.connectAttr( jointHierarchy[1] + '_crvBind.xformMatrix', ikh + '.dWorldUpMatrixEnd', f=1 )
	...
	if pm.objExists(anim_icon):
		pm.parent( ik_local, fk_local, anim_icon)
	else:
		pm.parent( ik_local, fk_local, rig_global_icon )
	...
	pm.parent( jointHierarchy[0] + '_fk', jointHierarchy[0] + '_ik', jointHierarchy[0] + '_drv', pad )
	...
	pm.parent( jointHierarchy[0] + '_crvBind', jointHierarchy[1] + '_crvBind', ikh, ik_crv, dnt_grp )
	...
	# Stretch tog
	pm.addAttr( ik_icon, ln='stretch', at='bool' )
	unlock_and_unhide( ik_icon, ['stretch'] )

	stretch_switch = pm.shadingNode( 'blendColors', asUtility=1, n=limbName + '_stretch_blend' )

	stretch_switch.color2R.set( arcLen )

	pm.connectAttr( ik_icon + '.stretch', stretch_switch  + '.blender', f=1 )
	...
	new_name = jointRoot.replace( '01_bind', '04_waste' )

	last_joint = pm.ls(jointHierarchy[3])[0]
	...
	print( 'Neck Setup Complete' )

def windowResize(*args):
	if pm.window('BR_bipedNeck_setup', q=1, exists=1):
		pm.window('BR_bipedNeck_setup', e=1, wh=(280, 80), rtf=1)
	else:
		pm.warning('BR_bipedNeck_setup does not exist')
